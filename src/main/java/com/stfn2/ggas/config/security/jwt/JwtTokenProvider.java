...
import com.stfn2.ggas.config.utils.JwtConfig;
...
public class JwtTokenProvider {
    ...
    public TokenVO createAccessToken(String username, List<String> roles) {
        Date now = new Date();
        Date validity = new Date(now.getTime() + jwtConfig.getExpireLength());
    ...
    private String getAccessToken(String username, List<String> roles, Date now, Date validity) {
        ...
        User user = userServices.getUserAuthenticated(username);
    ...
    private String getRefreshToken(String username, List<String> roles, Date now) {
        Date validityRefreshToken = new Date(now.getTime() + (jwtConfig.getExpireLength() * 3));
        return JWT.create()
                .withClaim("roles", roles)
                .withIssuedAt(now)
